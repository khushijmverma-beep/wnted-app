...
import { ChevronDown, ChevronLeft, Check, Mic, Plus, Search, Sparkles } from 'lucide-react';
...
export type DiscoverProduct = (typeof products)[number];
...
export default function SearchScreen({
  ...
  onToggleWishlist: (product: DiscoverProduct) => void;
}) {
  ...
  return (
    <div className="absolute inset-0 z-30 flex flex-col bg-transparent font-clash text-white">
      <div
        className={`flex h-full flex-col overflow-y-auto px-4 pt-10 pb-6 ${styles.discoverScroll}`}
      >
        <div className="mb-3 flex items-center gap-2">
          <button
            type="button"
            onClick={onClose}
            className="flex h-[34px] w-[34px] shrink-0 cursor-pointer items-center justify-center border-none bg-transparent p-0"
            aria-label="Back"
          >
        ...
        <div className={`${styles.categoryScroll} mb-[10px]`}>
        ...
        {filteredProducts.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            {filteredProducts.map((product) => {
              ...
              <div
                key={product.id}
                className={`relative flex flex-col overflow-hidden ${styles.glassWidget}`}
              ...
              >
                ...
                >
                  <button
                    ...
                    className="absolute right-1.5 top-1.5 z-10 flex h-7 w-7 cursor-pointer items-center justify-center rounded-full border-none transition-opacity"
                    style={{
